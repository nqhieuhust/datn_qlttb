    ...
    const handleSubmit = async () => {
        try {
            ...
            if (Object.keys(newErrors).length > 0) {
				...
				dispatch(toggleShowLoading(true));
                let data = { department_name: deName, address: address, manager: manager};
                const response = await departmentApi.createDepartment(data);
				await timeDelay(1000)
                if (response.status === 201 || response.status === 200 || response.status === 'success') {
    ...
    const handleUpdate = async (id) => {
        ...
        try {
            ...
            if (Object.keys(newErrors).length > 0) {
				...
				dispatch(toggleShowLoading(true));
                let data = { department_name: deNameEdit, address: addressEdit, manager: managerEdit};
                console.log(456);
            ...
            <Container fluid>
                <Row>
                    <Col md="12">
                        <Card className="strpied-tabled-with-hover">
                            <Card.Header style={{}}>
                                <Card.Title className={'d-flex justify-content-between'} as="h4">Danh sách Khoa/Phòng
								{role === 1 && <button onClick={() => setShowCre(true)} 
                                                ...
                                                departments.length > 0 ? (
                                                    departments.map((item, index) => (
                                                        <tr key={index}>
                                                            ...
                                                            <td className="text-break" style={ { minWidth: 265 } }>{item.department_name || 'N/A'}</td>
                                                            <td className="text-break" style={ { minWidth: 200 } }>{item.address || 'N/A'}</td>
                                                            <td className="text-break" style={ { minWidth: 250 } }>{item.manager || 'N/A'}</td>
															...
															<>
																...
																<td className="d-flex justify-between align-items-center">
																	...
																	onClick={() => { setShowModal(true); setIdDel(item.id) }}>Xóa</button>
                                                            	</td>
															</>
                ...
                <Modal show={showModal} dialogClassName="dialog-confirm">
                    ...
                    <Modal.Footer className="d-flex justify-content-center">
                        <button className="btn btn-primary" style={{ padding: '5px 20px', marginRight: 5 }} onClick={() => handleDelete(idDel)}>Đồng ý</button>
                        <button className="btn btn-secondary" style={{ padding: '5px 20px', marginLeft: 5 }} onClick={() => setShowModal(false)}>Hủy</button>
                    </Modal.Footer>
                </Modal>
                        ...
                        <Form>
                            <Form.Group>
                                ...
                                <Form.Control
                                    ...
                                    onChange={(e) => { setField('deName', e.target.value); setDeName(e.target.value) }}
                                    isInvalid={!!errors.deName}
                                />
                                {errors.deName && <span style={{ fontSize: 12, color: '#dc3545', fontWeight: 100 }}>{errors.deName}</span>}
                            </Form.Group>
                            ...
                            <Form.Group style={{ marginTop: 15 }}>
                                ...
                                <Form.Control
                                    ...
                                    onChange={(e) => { setField('address', e.target.value); setAdress(e.target.value) }}
                            ...
                            <Form.Group style={{ marginTop: 15 }}>
                                ...
                                />
                                {errors.manager && <span style={{ fontSize: 12, color: '#dc3545', fontWeight: 100 }}>{errors.manager}</span>}
                            </Form.Group>
                        </Form>
                    </Modal.Body>
                    <Modal.Footer style={{ justifyContent: 'center', marginTop: 15 }}>
                        <button type="submit" className="btn btn-primary" style={{ marginRight: 5, padding: '5px 10px' }} onClick={handleSubmit} disabled={loadingButton == true}>
                            {loadingButton === true &&
                        ...
                        <button type="button" className="btn btn-light" style={{ marginLeft: 5, padding: '5px 10px' }} onClick={handleClose}>
                            Hủy
                        </button>
                    </Modal.Footer>
                </Modal>
                ...
                <Modal show={edit}>
                    ...
                    {loadingForm === false &&
                        <>
                                ...
                                <Form>
                                    <Form.Group>
                                        ...
                                        <Form.Control
                                            ...
                                            placeholder="Nhập tên Khoa/Phòng"
                                            value={deNameEdit || ''}
                                            onChange={(e) => { setDeNameEdit(e.target.value); setField('deNameEdit', e.target.value) }}
                                            isInvalid={!!errors.deNameEdit}
                                        />
                                        {(errors.deNameEdit) && <span style={{ fontSize: 12, color: '#dc3545', fontWeight: 100 }}>{errors.deNameEdit}</span>}
                                    </Form.Group>
                                    ...
                                    <Form.Group style={{ marginTop: 15 }}>
                                        ...
                                        <Form.Control
                                            ...
                                            placeholder="Nhập địa chỉ"
                                            value={addressEdit || ''}
                                            onChange={(e) => { setAdressEdit(e.target.value); setField('addressEdit', e.target.value) }}
                                            isInvalid={!!errors.addressEdit}
                                        />
                                        {(errors.addressEdit) && <span style={{ fontSize: 12, color: '#dc3545', fontWeight: 100 }}>{errors.addressEdit}</span>}
                                    </Form.Group>
                                    ...
                                    <Form.Group style={{ marginTop: 15 }}>
                                        ...
                                        />
                                        {(errors.managerEdit) && <span style={{ fontSize: 12, color: '#dc3545', fontWeight: 100 }}>{errors.managerEdit}</span>}
                                    </Form.Group>
                            ...
                            <Modal.Footer style={{ justifyContent: 'center', marginTop: 15 }}>
                                <button type="submit" className="btn btn-primary" style={{ marginRight: 5, padding: '5px 10px', width: 90 }} onClick={() => handleUpdate(idEdit)}>
                                    {loadingButton === true &&
                                ...
                                <button type="button" className="btn btn-light" style={{ marginLeft: 5, padding: '5px 10px' }} onClick={handleEditOff}>
                                    Hủy
                                </button>
                            </Modal.Footer>
                        </>
