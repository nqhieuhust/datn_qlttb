...
function ProviderPage ()
{
	...
	const findFormErrors = ( type ) =>
	{
		...
		if ( type == 1 )
		{
			if ( !form.provider_name || form.provider_name === '' ) newErrors.provider_name = 'Nhà cung cấp không được để trống!';
			if ( !form.address || form.address === '' ) newErrors.address = 'Địa chỉ không được để trống!';
			if ( !form.representation || form.representation === '' ) newErrors.representation = 'Người đại diện không được để trống!';
			if ( !form.mobile || form.mobile === '' ) newErrors.mobile = 'Số điện thoại không được để trống!';
			else if ( !( form.mobile.match( '[0-9]{10}' ) ) ) newErrors.mobile = 'Số điện thoại không hợp lệ!';
			if ( !form.email || form.email === '' ) newErrors.email = 'Email không được để trống!';
			else if ( !/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}$/i.test( form.email ) ) newErrors.email = 'Email không hợp lệ!';
		}
		if ( type == 2 )
		{
			if ( !providerName || providerName === '' ) newErrors.provider_name = 'Nhà cung cấp không được để trống!';
			if ( !address || address === '' ) newErrors.address = 'Địa chỉ không được để trống!';
			if ( !representation || representation === '' ) newErrors.representation = 'Người đại diện không được để trống!';
			if ( !mobile || mobile === '' ) newErrors.mobile = 'Số điện thoại không được để trống!';
			else if ( !( mobile.match( '[0-9]{10}' ) ) ) newErrors.mobile = 'Số điện thoại không hợp lệ!';
			if ( !email || email === '' ) newErrors.email = 'Email không được để trống!';
			else if ( !/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}$/i.test( email ) ) newErrors.email = 'Email không hợp lệ!';
		}
				...
				<Row>
					<Col md="12">
						<Card className="strpied-tabled-with-hover">
							<Card.Header>
								<Card.Title className={ 'd-flex justify-content-between' } as="h4">Danh sách Nhà cung cấp<button onClick={ () => setShowCre( true ) } type="button" className="btn btn-info" style={ { padding: '6px 14px', fontSize: 14 } }><span>Thêm mới</span></button></Card.Title>
								<div className="my-4">
									<FilterProvider
